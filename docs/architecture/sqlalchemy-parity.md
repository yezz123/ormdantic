...
| DDL                                   | Partial | Tables, keys, indexes, and schema-diff SQL are covered; SQLite rebuild-required alters are blocked until automatic rebuild planning lands.               |
...
| Migrations                            | Partial | Artifact V2 checksums, durable history (status/dirty/checksum), explicit rollback, and status/history/current/repair/check CLI commands are available.   |
| Reflection                            | Partial | Runtime inspection is available and SQLite live-autogenerate is supported; full cross-dialect live autogenerate remains planned.                         |
