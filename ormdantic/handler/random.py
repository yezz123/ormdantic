...
from ormdantic._introspect import FieldMetadata
from ormdantic.types import AnyNumber, default_max_length


def _random_str_value(model_field: FieldMetadata) -> str:
    """Get a random string."""
    target_length = _get_target_length(
        model_field.min_length, model_field.max_length
    )
    choices = string.ascii_letters + string.digits
    return _random_str(choices, target_length)


def _random_number_value(model_field: FieldMetadata) -> AnyNumber:
    """Get a random number."""
